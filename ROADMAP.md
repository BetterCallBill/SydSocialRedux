...
| T-008 | P1 | Version-control Firestore/Storage security rules | Security | Blocked | No `firestore.rules`/`storage.rules` exist in this repo (there is no `firebase.json` at all now that hosting moved to GitHub Pages — see README), so rules can only be reviewed/audited by pulling them from the live Firebase Console. Add the rules files to the repo (pulled from current prod config) and deploy them via `firebase deploy --only firestore:rules,storage:rules` in a dedicated CI step | Requires a human to export the current live rules first (`firebase firestore:rules:get` / console) — the loop cannot fabricate security rules for a live production database |
