...
| T-007a | P3 | Migrate ESLint config to flat config + ESLint 9/10 | Dependency health | Not Started | `.eslintrc.cjs` (legacy config) replaced by `eslint.config.js`; `@typescript-eslint` bumped to v8 to match. Dev-tooling only, no runtime risk — safe for the loop to attempt unattended | |
