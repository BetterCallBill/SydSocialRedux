...
| T-010 | P1 | Fix ref accessed during render in EventFilters | Error handling & resilience | Not Started | Found by `react-hooks/refs` (part of `eslint-plugin-react-hooks` v7, deliberately not enabled repo-wide in T-007a — see eslint.config.js comment). `src/features/events/dashboard/EventFilters.tsx:79` reads `startDate.current` during render, which can cause stale UI. Refactor to use state instead of a ref for the date shown in the picker | |
| T-011 | P2 | Fix direct state mutation in ProfileEvents | Error handling & resilience | Not Started | Found by `react-hooks/immutability`. `src/features/profiles/ProfileEvents.tsx:53` mutates `initialOptions` (state) directly (`options.reset = true`) before calling `setOptions`. Copy before mutating | |
| T-012 | P3 | Investigate react-hook-form `watch()` compiler-incompatibility warning | Code quality / architecture | Not Started | Found by `react-hooks/incompatible-library`. `src/features/auth/AccoutPage.tsx:25` — `watch()` returns a value React Compiler can't safely memoize. Not urgent (app doesn't use React Compiler yet), but worth a look before T-007b/React 19 upgrade | |
